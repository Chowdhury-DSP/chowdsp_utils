...
template <int maxNumModes, typename SampleType>
void ModalFilterBank<maxNumModes, SampleType>::setNumModesToProcess (int newNumModesToProcess)
{
...
void ModalFilterBank<maxNumModes, SampleType>::process (const juce::dsp::AudioBlock<const SampleType>& block) noexcept
{
    ...
    for (size_t modeIdx = 0; modeIdx < (size_t) numVecModesToProcess; ++modeIdx)
    {

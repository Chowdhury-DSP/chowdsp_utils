...
TEST (xsimd, avx512_float_hyperbolic)
{
    std::ofstream out ("log/avx512_float_hyperbolic.log", std::ios_base::out);
    bool res = xsimd::test_hyperbolic<float, 16, 64> (out, "avx512 float");
    EXPECT_TRUE (res);
}

TEST (xsimd, avx512_double_hyperbolic)
{
    std::ofstream out ("log/avx512_double_hyperbolic.log", std::ios_base::out);
    bool res = xsimd::test_hyperbolic<double, 8, 64> (out, "avx512 double");
    EXPECT_TRUE (res);
}
#endif
...
TEST (xsimd, neon_double_hyperbolic)
{
    ...
    bool res = xsimd::test_hyperbolic<double, 2, 32> (out, "neon double");
    EXPECT_TRUE (res);
}
#endif

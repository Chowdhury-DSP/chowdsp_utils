...
namespace Polynomials
{
    ...
    inline P<T, X> naive (const T (&coeffs)[ORDER + 1], const X x)
    {
     ...
     */
    template <int ORDER, typename T, typename X>
    inline P<T, X> horner (const T (&coeffs)[ORDER + 1], const X x)
    {
        P<T, X> b = coeffs[0];
     ...
     */
    template <int ORDER, typename T, typename X>
    inline P<T, X> estrin (const T (&coeffs)[ORDER + 1], const X x)
    {

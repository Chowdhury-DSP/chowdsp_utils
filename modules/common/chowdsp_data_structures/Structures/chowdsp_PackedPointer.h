...
struct PackedPointer
{
    ...
    void set (T* ptr, uint8_t flags = 0)
    {
        ...
        raw_ptr = reinterpret_cast<T*> ((reinterpret_cast<uintptr_t> (ptr) & address_mask)
    ...
    {
        jassert (flags <= max_flag_value);
        raw_ptr = reinterpret_cast<T*> ((reinterpret_cast<uintptr_t> (raw_ptr) & address_mask)
                                        | (static_cast<uintptr_t> (flags) & flags_mask));
    }

    [[nodiscard]] T* get_ptr()
    {
        return reinterpret_cast<T*> (reinterpret_cast<uintptr_t> (raw_ptr) & address_mask);
    }
    ...
    {
        return static_cast<uint8_t> (reinterpret_cast<uintptr_t> (raw_ptr) & flags_mask);
    }

...
using range_formatter_type =
    conditional_t<is_formattable<Element, Char>::value,
                  formatter<remove_cvref_t<decltype(
                                range_mapper<buffer_context<Char>>{}.map(std::declval<Element>()))>,
                            Char>,
...
public:
    ...
    FMT_CONSTEXPR auto parse(ParseContext& ctx) -> decltype(ctx.begin()) {
        ...
        if (*it != ':') FMT_THROW(format_error("no other top-level range formatters supported"));

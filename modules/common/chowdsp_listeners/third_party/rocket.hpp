...
public:
    ...
    stable_list (stable_list&& l)
        : head { std::move (l.head) }, tail { std::move (l.tail) }, elements { l.elements }
    {
        l.init();
    }
...
namespace detail
{
    ...
    struct connection_base<thread_unsafe_policy>
        : thread_unsafe_policy, ref_counted<connection_base<thread_unsafe_policy>>
    {
    ...
    struct connection_base<thread_safe_policy>
        : thread_safe_policy, ref_counted<connection_base<thread_safe_policy>, ref_count_atomic>
    {
        virtual ~connection_base() ROCKET_NOEXCEPT = default;
...
template <class Signature, class Collector = default_collector<detail::get_return_type<Signature>>, class ThreadingPolicy = thread_unsafe_policy>
struct signal;

...
/** isNAN benchmarks............................ */
constexpr int countNanN = 10000;
const auto countNanVecFloat = bench_utils::makeRandomVector<float> (countNanN);
...
static void countNanSTLFloat (benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        auto nanCount = idealCountNan (countNanVecFloat.data(), countNanN);
        benchmark::DoNotOptimize (nanCount);
    }
}
BENCHMARK (countNanSTLFloat);

static void countNanChowFloat (benchmark::State& state)
{
    for (auto _ : state)
    {
        auto nanCount = chowdsp::FloatVectorOperations::countNaNs (countNanVecFloat.data(), countNanN);
        benchmark::DoNotOptimize (nanCount);
    }
}
BENCHMARK (countNanChowFloat);

/** isInf benchmarks............................ */
constexpr int countInfN = 10000;
const auto countInfVecFloat = bench_utils::makeRandomVector<float> (countInfN);
...
static void countInfSTLFloat (benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        auto infsCount = idealCountInf (countInfVecFloat.data(), countInfN);
...
static void countInfChowFloat (benchmark::State& state)
{
    for (auto _ : state)
    {
        auto infsCount = chowdsp::FloatVectorOperations::countInfs (countInfVecFloat.data(), countInfN);
        benchmark::DoNotOptimize (infsCount);
    }
}

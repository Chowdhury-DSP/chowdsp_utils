...
public:
    ...
    template <typename T>
    void checkResult (T result_scalar, juce::dsp::SIMDRegister<T> result_vec, const juce::String& mathOpName, const juce::String& opType, T maxErr)
    {
    ...
    template <typename T>
    void mathTest (juce::Random& r, int nIter, T maxError)
    {
    ...
    void fastExpTest()
    {
        ...
        angles[0] = 0;
        angles[1] = juce::MathConstants<T>::pi / (T) 2;
        if constexpr (std::is_same<T, float>::value)
        {
            angles[2] = (T) 3 * juce::MathConstants<T>::pi / (T) 4;
            angles[3] = juce::MathConstants<T>::pi;
        }
        auto asse = juce::dsp::SIMDRegister<T>::fromRawArray (angles);

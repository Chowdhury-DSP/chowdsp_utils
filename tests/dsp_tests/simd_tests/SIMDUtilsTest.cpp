...
public:
    ...
    void baseMathTest (int nIter, std::function<FloatType (FloatType)> floatFunc, std::function<dsp::SIMDRegister<FloatType> (dsp::SIMDRegister<FloatType>)> simdFunc,
                       FloatType maxErr, String functionName, const NormalisableRange<FloatType>& range)
    {
...
#define SIMDFUNC(func) [] (dsp::SIMDRegister<FloatType> x) { return func (x); }
        ...
        baseMathTest<FloatType> (nIter, FLOATFUNC (std::tan), SIMDFUNC (tanSIMD), maxErr, "tan", minus10To10);
        baseMathTest<FloatType> (nIter, FLOATFUNC (std::sinh), SIMDFUNC (sinhSIMD), maxErr, "sinh", minus10To10);
        baseMathTest<FloatType> (nIter, FLOATFUNC (std::cosh), SIMDFUNC (coshSIMD), maxErr, "cosh", minus10To10);
